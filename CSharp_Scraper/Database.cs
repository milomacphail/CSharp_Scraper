...
namespace CSharp_Scraper
{
    class Database
    {
        private const string _connection = @"Data Source=(localdb)\MSSQLLocalDB;Initial Catalog=StockScrapes;Integrated Security=True;Connect Timeout=30;Encrypt=False;TrustServerCertificate=False;ApplicationIntent=ReadWrite;MultiSubnetFailover=False";
        ...
        {
            string lastScrape = @"IF EXISTS(SELECT* FROM StockScrape WHERE Stock_Symbol = @stock_symbol)
                                    UPDATE StockScrape
                                ...
                                ELSE 
                                    INSERT INTO StockScrape VALUES (@time_scraped, @stock_symbol, @last_price, @change, @change_percent, @volume, @shares, @average_volume, @market_cap);";
        ...
        private static void DataToTable(Stock stock)
        {
            ...
            using (SqlConnection db = new SqlConnection(_connection))
            {
                string insertToTable = "INSERT INTO dbo.StockScrape (Time_Scraped, Stock_Symbol, Last_Price, Change, Change_Percent, Volume, Shares, Average_Volume, Market_Cap) VALUES (@time_scraped, @stock_symbol, @last_price, @change, @change_percent, @volume, @shares, @average_volume, @market_cap);";
                {            
                    db.Open();
